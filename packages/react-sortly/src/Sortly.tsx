...
const getElConnectableElement = (connectedDropTarget?: Connectable | React.RefObject<Connectable | undefined>) => {
  ...
  const el = isRef(connectable) ? (connectable as React.RefObject<Element>).current : (connectable as Element);
...
function Sortly<D = { id: ID }>(props: SortlyProps<D>) {
  ...
  const handleHoverBegin = React.useCallback((id: ID, connectedDropTarget?: React.MutableRefObject<Connectable | undefined>) => {
